...
async function handleExportGuild(message) {
  ...
  const autoSaveTimer = setInterval(() => {
    if (!exportState.saveInProgress) {
      saveExportProgress(exportState);
    }
  }, AUTO_SAVE_INTERVAL);
  ...
  try {
    ...
    await saveExportProgress(exportState);
...
async function saveExportProgress(exportState) {
...
async function updateStatusMessage(statusMessage, exportState, guild, isFinal = false) {
  ...
  // Calculate export number
  const exportNumber = Math.ceil(exportState.processedMessages / EXPORT_THRESHOLD) || 1;
